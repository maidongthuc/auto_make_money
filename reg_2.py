...
url = "https://mproxy.vn/capi/aimwlugm_5WhOej62Q9_JN57ct4vFhCBP6bTTcjGUKw/key/SvreU215UuiBFW/resetIp"
data = pd.read_csv('E:/SELENIUM/CWIN/REG_2/data.csv', header=None).values
...
for i in range(41,50):
    ...
    sleep(3)
    input_text(device, 'https://www.whatismyip.com/')
    sleep(1)
    press_enter(device)
    sleep(1)
    ...
    sleep(1)
    input_text(device, 'SvreU215UuiBFW')
    sleep(1)
    tap(device, 493, 800) # nhấn login
    sleep(15)

    tap(device, 616, 91) # nhấn 1
    sleep(1)
    tap(device, 616, 91) # nhấn 1
    sleep(1)
    input_text(device, 'https://cwin988.com/')
    sleep(1)
    press_enter(device)
    sleep(15)
    tap(device, 597, 714) # nhấn link
    sleep(15)
    tap(device, 595, 755) # nhấn đóng
    sleep(1)
...
# y2 = 250
# duration = 1000  
# swipe(device, x1, y1, x2, y2, duration)
